...
pub fn model_loop(thread_running: Arc<AtomicBool>, game_objects: Arc<RwLock<Vec<Box<dyn GameObject + Send + Sync>>>>, static_objects : Arc<RwLock<Vec<StaticObject>>>){
    ...
    construct_game_logic(game_objects.clone(), static_objects.clone());
...
fn construct_game_logic(game_objects: Arc<RwLock<Vec<Box<dyn GameObject + Send + Sync>>>>, static_objects : Arc<RwLock<Vec<StaticObject>>>){
