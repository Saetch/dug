...
fn start_threads()-> (Vec<JoinHandle<()>>, flume::Sender<ControllerInput>, single_value_channel::Receiver<Option<Vec<Vertex>>>, Arc<AtomicBool>){
    ...
    let (render_receiver, render_sender) = single_value_channel::channel::<Vec<Vertex>>();
...
fn create_game_structs() -> (Arc<RwLock<GameState>>, Arc<RwLock<Vec<Box<dyn GameObject + Send + Sync>>>>, Arc<RwLock<Vec<StaticObject>>>){
    ...
    let game_state_arc = Arc::new(RwLock::new(GameState::new()));
